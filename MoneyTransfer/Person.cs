    ...
    internal class Person
    {
        ...
        public bool TransferMoney(Person receiver, int amountToTransfer)
        {
            ...
            if(amountToTransfer <=wallet.money) 
            {
                ...
                Console.WriteLine($"{name} has transferred ${amountToTransfer} to Brock and has ${wallet.money} remaining in their account");
                return true;
            }
                else
            {
                Console.WriteLine($"insufficient funds for transfer, you have {wallet.money} and are trying to transfer {amountToTransfer} to {receiver}");
                return false;
            }
